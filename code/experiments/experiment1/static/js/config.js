...
var Config = function (condition, counterbalance) {
    ...
    this.parse_config = function (data) {
        this.trials = shuffle(data["features"])
        this.names = shuffle(data["names"])
        this.features = data["features"]
        this.feature_text = data["feature_text"]
        this.name_text = data["name_text"]
        this.question = data["question"]
        this.feature_categories = shuffle(data["feature_categories"])
    };


    // this.parse_config = function (data) {
    //     var questions = ["category", "typicality", "purpose", "H2O"]
    //     var items = ["water_items", "non_water_items"]
    //     this.item_type = items[that.condition]
    //     this.question_info = data["question_type"][questions[that.counterbalance]]["question"]
    //     this.items = shuffle(data["item_type"][this.item_type])
    // };
